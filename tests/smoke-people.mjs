...
// --- An ITP: the same bar, pinned at the foot of the ITP page.
await tab(page, 'Controldoc').click()
await page.waitForTimeout(400)
await page.getByRole('button', { name: /ITP register/ }).click()
await page.getByPlaceholder(/Search the \d+/).fill('023')
await page.waitForTimeout(300)
await page.locator('.listitem').first().click()
await page.getByPlaceholder(/North East Corner/).fill('Level 20 cold water')
await page.getByRole('button', { name: 'Raise ITP' }).click()
await page.waitForTimeout(800)
const bar = page.locator('.pagefoot')
await bar.getByRole('button', { name: 'Allocate to worker' }).waitFor()
check(await bar.getByRole('button', { name: 'Save' }).isVisible(), 'No Save button on the ITP')
await bar.getByRole('button', { name: 'Allocate to worker' }).click()
await bar.getByPlaceholder("Worker's name").fill('Joe Bloggs')
await bar.locator('label:has(span:text("Wanted by")) input').fill('2099-11-15')
await bar.getByRole('button', { name: /^Allocate ITP / }).click()
await page.waitForTimeout(500)
const itpEvent = await lastEvent('itp.allocated')
console.log('ITP event:', JSON.stringify({ to: itpEvent?.notifyEmails, link: itpEvent?.link, summary: itpEvent?.summary }))
check(itpEvent?.notifyEmails === 'joe.bloggs@axis.test' && /#\/project\/[^/]+\/itp\/itp_/.test(itpEvent?.link ?? ''), 'ITP allocation not addressed to Joe with a link to the ITP')
const itp = (await db('itps'))[0]
check(itp.assignedTo === 'Joe Bloggs' && itp.assignDue === '2099-11-15', 'Allocation not stored on the ITP')
await shot('03-itp-bar')
await page.screenshot({ path: path.join(OUT, '03b-itp-bar-screen.png') })
await bar.getByRole('button', { name: 'Save' }).click()
await page.locator('.toast', { hasText: /saved/ }).waitFor()
check(/\/itps$/.test(page.url()), `ITP Save did not return to the register: ${page.url()}`)
await page.locator('.listitem', { hasText: '→ Joe Bloggs' }).waitFor({ timeout: 5000 }).catch(() => errors.push('ITP register does not show who it is allocated to'))
